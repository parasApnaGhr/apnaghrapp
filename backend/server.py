...
async def get_my_bookings(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # Batch collect all rider IDs and property IDs
        rider_ids = set()
        property_ids = set()
        for booking in bookings:
            rid = booking.get('rider_id') or booking.get('assigned_rider_id')
            if rid:
                rider_ids.add(rid)
            prop_ids = booking.get('property_ids', [])
            if not prop_ids and booking.get('property_id'):
                prop_ids = [booking.get('property_id')]
            for pid in prop_ids[:1]:  # Only first property
        ...
        for booking in bookings:
            ...
            if rider_id and rider_id in rider_map:
            ...
            if not prop_ids and booking.get('property_id'):
                prop_ids = [booking.get('property_id')]
            if prop_ids and prop_ids[0] in prop_map:
                first_prop = prop_map[prop_ids[0]]
