...
class Lead(BaseModel):
    ...
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    # Lead source info
    source: str  # app_visit, property_view, property_click, visit_booking, seller_referral, image_click
    # Contact info
    ...
    user_role: Optional[str] = None  # customer, seller, etc.
    # Seller referral info
    ...
    # Lead status
    status: str = "new"  # new, contacted, interested, not_interested, converted, closed
    notes: Optional[str] = None
...
class BuilderProject(BaseModel):
    ...
    phase: str  # pre_pre_launch, pre_launch, launched
    description: str
    ...
    unit_types: List[str] = []  # 1BHK, 2BHK, 3BHK, Villa, Plot, etc.
    ...
    # Pre-Launch Specific
    clu_status: Optional[str] = None  # pending, approved, in_process
    ...
    subscription_plan: str = "none"  # none, pre_pre_launch, standard
    ...
    # Commission
    commission_percent: float = 2.0  # Default 2%
...
class BuilderEvent(BaseModel):
    """Events for Pre-Pre Launch projects - Investor meetups"""
    model_config = ConfigDict(extra="ignore")
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    project_id: str
    builder_id: str
    ...
    event_name: str
    event_type: str  # investor_meetup, site_visit, webinar, launch_event
    description: str
...
class BuilderEventRegistration(BaseModel):
    """Registration for builder events"""
    model_config = ConfigDict(extra="ignore")
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    event_id: str
    project_id: str
    ...
    phone: str
    email: Optional[str] = None
    investment_capacity: Optional[str] = None  # 10-25L, 25-50L, 50L-1Cr, 1Cr+
    
    # Status
    status: str = "registered"  # registered, confirmed, attended, no_show, cancelled
    attended: bool = False
    
    # Source
    source: str = "apnaghr"  # apnaghr, referral, direct
...
class BuilderProjectLead(BaseModel):
    """Leads/Inquiries for builder projects"""
    model_config = ConfigDict(extra="ignore")
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    project_id: str
    builder_id: str
    ...
    # Source
    source: str  # click, inquiry_form, site_visit, event, referral
    page_url: Optional[str] = None
    
    # Site Visit (if scheduled)
    site_visit_scheduled: bool = False
    site_visit_date: Optional[str] = None
    site_visit_status: Optional[str] = None  # scheduled, completed, cancelled, no_show
    rider_id: Optional[str] = None
    
    # Status
    status: str = "new"  # new, contacted, interested, site_visit, negotiation, converted, lost
    notes: Optional[str] = None
...
async def create_builder_project(project_data: BuilderProjectCreate, current_user: dict = Depends(get_current_user)):
    ...
    # Set subscription based on phase
...
@api_router.get("/builder/projects/{project_id}/leads")
async def get_project_leads(project_id: str, current_user: dict = Depends(get_current_user)):
    ...
    # Verify project belongs to builder
...
@api_router.patch("/builder/projects/{project_id}/leads/{lead_id}")
...
# Builder Events (Pre-Pre Launch)
@api_router.post("/builder/events")
async def create_builder_event(event_data: dict, current_user: dict = Depends(get_current_user)):
    ...
    # Verify project exists and is pre-pre launch
    project = await db.builder_projects.find_one({
...
@api_router.get("/builder/events/{event_id}/registrations")
...
# Public endpoints for builder projects
@api_router.get("/projects/public")
...
@api_router.get("/projects/public/{project_id}")
...
@api_router.post("/projects/public/{project_id}/inquiry")
...
@api_router.post("/projects/public/{project_id}/click")
...
# Admin endpoints for builder projects
@api_router.get("/admin/builder-projects")
...
@api_router.patch("/admin/builder-projects/{project_id}")
